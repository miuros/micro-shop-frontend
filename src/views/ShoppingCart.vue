        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          ...
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.num"
              @change="handleChange($event,index,item.id,item.productId)"
              :min="1"
            ></el-input-number>
          </div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.id,item.id,item.id,item.productId)"
                >确定</el-button>
              </div>
              <i class="el-icon-error" slot="reference" style="font-size: 18px;"></i>
            </el-popover>
          </div>
          <div class="pro-commit">
            <el-button @click="commit(item.productId,item.num)">提交</el-button>
          </div>
        </li>
        <!-- 购物车列表END -->
      </ul>
  ...
  methods: {
    ...
    commit(id,num){
      if(this.arList.length===0){
        this.notifyError('请先添加地址');
        return;
      }
      this.$axios.post("/api/od/v1/create",JSON.stringify([{
        productId: id,
        number: num,
        addressId: this.arList[0].id,
      }]),{
        headers: {
    ...
    // 修改商品数量的时候调用该函数
    handleChange(currentValue, key,id, productID) {
      // 当修改数量时，默认勾选
      this.updateShoppingCart({ key: key, prop: "check", val: true });
      // 向后端发起更新购物车的数据库信息请求
      this.$axios
        .put("/api/ct/v1/update", JSON.stringify({
          id: parseInt(id),
          productId: parseInt(productID),
          num: parseInt(currentValue)
        }),{
          headers: {
            "Content-Type":"application/json",
          }
...
.shoppingCart .content ul .pro-commit{
  float: left;
  width: 120px;
  text-align: center;
}
