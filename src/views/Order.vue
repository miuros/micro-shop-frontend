      ...
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <li class="order-info">
            ...
            <div class="order-time">订单时间: {{item.createAt }}</div>
          </li>
          <li class="header">
            <div class="pro-img"></div>
            <div class="pro-name">商品ID</div>
            <div class="pro-num">数量</div>
            <div class="pro-total">小计</div>
            <div class="pro-status">状态</div>
            <div class="pro-commit">操作</div>
          </li>
          <!-- 我的订单表头END -->
          ...
          <li class="product-list" >
            ...
            <div class="pro-name">
              ...
              >{{item.productId}}</router-link>
            </div>
            
            <div class="pro-num">{{item.number}}</div>
            <div class="pro-total">{{item.price}}元</div>
            <div class="pro-status">{{item.status}}</div>
            <div class="pro-commit" v-if="item.status==1"><el-button @click="payorder(item.id)">支付</el-button><el-button @click="deleteOrder(item.id)">删除</el-button></div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods:{
    deleteOrder(id){
      this.$axios.delete("api/od/v1/delete?id="+id).then(res=>{
    ...
    payorder(id){
      this.$axios.post("/api/od/v1/pay",this.$qs.stringify({
        id: id,
      })).then(res=>{
        if (res.data.code==='200'){
          this.notifySucceed(res.data.msg);
